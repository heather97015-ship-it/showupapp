...
import { getJob, confirmAttendance, markNoShow, deployBackup, completeJob } from "~/lib/jobs";
...
function JobDetail() {
  ...
  const [deployingBackup, setDeployingBackup] = useState(false);
  ...
  async function handleNoShow() {
    ...
    try {
      ...
      setActionResult("✗ Marked as no-show. Penalty applied.");
      setTimeout(() => window.location.reload(), 800);
    } catch (err) {
      setActionError(err instanceof Error ? err.message : "Failed");
    }
  ...
  async function handleDeployBackup() {
    setDeployingBackup(true);
    setActionError(null);
    setActionResult(null);
    try {
      const result = await deployBackup({ data: { job_id: job.id } });
      if (result.success) {
        setActionResult(`✓ Backup deployed: ${result.backup_cleaner_name}`);
        setTimeout(() => window.location.reload(), 1000);
      } else {
        setActionError(result.error ?? "No backup available");
      }
    } catch (err) {
      setActionError(err instanceof Error ? err.message : "Failed to deploy backup");
    } finally {
      setDeployingBackup(false);
    }
  }
    ...
    <div className="max-w-2xl mx-auto px-4 py-6 space-y-6">
      ...
      {job.status !== "completed" && job.status !== "no_show" && (
        <div className="space-y-3">
          {/* Confirm / No-show for assigned cleaner */}
          {job.assigned_cleaner_id && !job.attendance_confirmed && job.status === "pending" && (
            <div className="flex gap-2">
              <button
                onClick={handleConfirm}
                className="flex-1 bg-green-600 text-white rounded-lg py-3 text-sm font-medium hover:bg-green-700"
              >
                ✓ Confirm Attendance
              </button>
              <button
                onClick={handleNoShow}
                className="flex-1 bg-red-600 text-white rounded-lg py-3 text-sm font-medium hover:bg-red-700"
              >
                ✗ Mark No-Show
              </button>
            </div>
          ...
          {/* Deploy backup */}
          {!job.backup_cleaner_id && (
            <button
              onClick={handleDeployBackup}
              disabled={deployingBackup}
              className="w-full bg-orange-600 text-white rounded-lg py-3 text-sm font-medium hover:bg-orange-700 disabled:opacity-50"
            >
              {deployingBackup ? "Finding backup..." : "🔄 Deploy Backup Cleaner"}
            </button>
          )}
